"""Tests for OpenAILLMProvider implementation."""
...
class TestOpenAILLMProvider:
    ...
    @pytest.fixture
    def provider(self):
        """Create OpenAILLMProvider with test API key."""
        os.environ["OPENAI_API_KEY"] = "test-openai-api-key"
        return OpenAILLMProvider(api_key="test-openai-api-key")
    ...
    def test_provider_custom_model(self):
        ...
        os.environ["OPENAI_API_KEY"] = "test-openai-api-key"
        provider = OpenAILLMProvider(api_key="test-openai-api-key", model="gpt-4o")
    ...
    def test_supports_json_mode(self, provider):
        """OpenAI supports JSON response format."""
    ...
    async def test_generate_returns_llm_response(self, provider):
        ...
        mock_choice.message.content = "OpenAI GPT response text"
        ...
        with patch("src.providers.llm.openai_llm.OpenAI", return_value=mock_client):
            result = await provider.generate("Hello OpenAI")

        assert isinstance(result, LLMResponse)
        assert result.text == "OpenAI GPT response text"
    ...
    async def test_generate_with_system_prompt(self, provider):
        ...
        assert messages[0]["content"] == "You are a summarization expert."
